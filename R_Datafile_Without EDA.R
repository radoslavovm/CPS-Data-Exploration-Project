...
ggplot() +
  geom_bar(
    aes(
      x = reorder(names(loadings2), 
                  abs(loadings2)),
      y = loadings), stat = "identity") +
  coord_flip() +
  labs(title = paste("Loadings for the Second PC"), x = "Variable", y = "Loading")
...
m <- round(nrow(df) * 0.1) # Sample 10% of rows for Hopkins statistic
...
# PCA
# Complete rows used for PCA
rows_used <- complete.cases(data %>% select(-NAME))

# Create the full-length PC vectors
PC1_full <- rep(NA, nrow(data))
PC2_full <- rep(NA, nrow(data))

PC1_full[rows_used] <- pc_scores$PC1
PC2_full[rows_used] <- pc_scores$PC2

# Create a dataset with PC1 and PC2 values full dataset
pca_df <- acs_clean %>%
  mutate(
    PC1 = PC1_full,
    PC2 = PC2_full
...
cluster_vec <- rep(NA, nrow(acs_clean))
...
ggplot(map_data_cluster) +
  ...
  labs(
    title = "K-Means Clusters of Texas Counties",
    fill = "Cluster"
  ) +
  theme_void()
